...
function wippl_and_grad!(grad::Vector{Float64}, x::Vector{Float64}, plmvar::AttPlmVar)
    ...
    data = wipAttComputationQuantities(N,H,q)

    grad .= 0.0
     
    Threads.@threads for site in 1:N 
        pseudologlikelihood[site], reg[site] = wipupdate_QK_site!(grad, Z, view(Q,:,:,site), K, V, site, weights, λ, data,view(delta,site,:,:),wdelta)
    end
...
function wipupdate_QK_site!(grad::Vector{Float64}, Z::Array{Int,2}, Q::AbstractArray{Float64,2}, K::Array{Float64,3}, V::Array{Float64,3}, site::Int, weights::Vector{Float64}, lambda::Float64, data::wipAttComputationQuantities, delta, wdelta)
...
function wipupdate_V!(grad::Vector{Float64}, Q::Array{Float64,3}, V::Array{Float64,3}, lambda::Float64, data::wipAttComputationQuantities)
