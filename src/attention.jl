function attentiondca(Z::Array{T,2}, Weights::Vector{Float64};
    ...
    lambdaJ::Real = version == :NOFIELD ? 0.01 : 0.01, 
...
function pl_and_grad!(grad::Vector{Float64}, x::Vector{Float64}, plmvar::FieldAttPlmVar)
    ...
    @extract plmvar : H N M d dd q Z λJ = lambdaJ λH = lambdaH weights = W delta wdelta
    L = 2*H*N*d + H*q*q + N*q
    ...
    Threads.@threads for site in 1:N 
        pseudologlikelihood[site], reg[site], big_scra_grad[site,:]= update_QK_site!(grad, Z, view(Q,:,:,site), K, V, view(F,:,site),site, weights, λJ,λH, data,view(delta,site,:,:),wdelta,dd)
    end
...
function update_QK_site!(grad::Vector{Float64}, Z::Array{Int,2}, Q::AbstractArray{Float64,2}, K::Array{Float64,3}, V::Array{Float64,3}, F, site::Int, weights::Vector{Float64}, lambdaJ::Float64, lambdaH::Float64, data::AttComputationQuantities, delta::AbstractArray{Int, 2}, wdelta::Array{Float64,3}, dd::Real)
    H,d = size(Q)
    H,q,_ = size(V)
    N,M = size(Z) 
    @tullio sf[j, h] := Q[h,d]*K[h,d,j]
